(function (global) {
    ...
    function render(ctx, state, point) {
        ...
        //tx.drawImage(canv_cache.canv_effect, 0, 0, RECT_CANV.w, RECT_CANV.h);
        ...
        for (var x = 0; x < COL; x++) {
            for (var y = 0; y < COL; y++) {
                if (state.map[y * COL + x] != 0) {
                    drawPiece(ctx, x * CELL_SIZE, y * CELL_SIZE, state.map[y * COL + x],state.point);
                }
            }
        }
    ...
    function drawPiece(ctx, x, y, number,point) {
        ...
        if (number == 1) {
            ...
            grad.addColorStop(0, 'rgb(180, 180, 180)');
            grad.addColorStop(0.4, 'rgb(180, 180, 180)');
        ...
        ctx.arc(x + CELL_SIZE / 2,  y+ CELL_SIZE / 2, CELL_SIZE/2, 0, Math.PI*2, false);
        ctx.fill()
        if(number==1){
            ctx.fillStyle=COLOR_BLACK;
            ctx.font = "18px 'ＭＳ Ｐゴシック'";
            ctx.fillText(point, x+CELL_SIZE/2.5, y+CELL_SIZE/1.65 )
        }
        if(number==2){
            ctx.fillStyle=COLOR_BLACK;
            ctx.font = "18px 'ＭＳ Ｐゴシック'";
            ctx.fillText("ｴｻ", x+CELL_SIZE/2.8, y+CELL_SIZE/1.65 )
        }
